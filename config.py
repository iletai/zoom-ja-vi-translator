...
_ROUTER_DEFAULT_PROMPT = (
    ...
    "(ね, よ, が, は, を, に, も) output '...'. But if the input has any real "
    "content (even a short lead-in like ちょっとその件は), translate it and let it "
    "trail off — do NOT collapse a content fragment to bare '...'. A lone "
    "question-ender (か, ですか, でしょうか, かな) is a QUESTION, not filler — render "
    "it as '...không ạ?' / '...phải không ạ?', never as bare '...'.\n"
    ...
    "- Prior turns in this conversation are shown before the current line. Use them "
    "for consistency: translate a recurring name, term, or topic the SAME way you "
    "did earlier, and resolve an ambiguous fragment using what was just said. Only "
    "ever output the translation of the CURRENT line, never re-translate prior turns.\n"
    "- First person 自分/私 = 'tôi' (I), never 'bạn' (you).\n"
    "- Short back-channels (はい, うん, ええ, なるほど, えっと) are handled upstream; "
    "if one slips through, render it as a brief acknowledgement. Do NOT prepend "
    "'Vâng'/'Dạ' to a sentence that has real content — a discourse opener like "
    "まあ, そう, よ, ええと is NOT agreement; translate the actual words, never add "
    "an acknowledgement the speaker did not say.\n"
